...
--edit this to represent your own (DCS cant write to different disks)
csvFilePath = "C:\\HypeMan\\PersianFury\\db.csv" 
...
if file_exists("anapa_sceneryTgtList.lua") then
	inZone = table.load("anapa_sceneryTgtList.lua")
else
	inZone = {}
end
...
function EH:OnEventDead( EventData )
	if EventData.IniUnit and EventData.IniObjectCategory==Object.Category.SCENERY then
		...
		if table_has_key(inZone, EventData.initiator.id_) then
			HypeMan.sendDebugMessage(' EventData.initiator.id_ was in the list of targets')
...
function EH1:OnEventMarkRemoved(EventData)
		 ...
		 elseif EventData.text:lower():find("82") then
			EventData.MarkCoordinate:Explosion(110)
		 elseif EventData.text:lower():find("83") then
			EventData.MarkCoordinate:Explosion(202)
		 elseif EventData.text:lower():find("84") then		 
			EventData.MarkCoordinate:Explosion(340)
