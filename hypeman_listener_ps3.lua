...
-- Grow a list of recon reports.  It's a bit unpleasant that
-- this will continue to grow, but deleting fields from lua tables
-- is surprisingly hard
local reconDB = {}
local function AppendReconMessage(tbl)  
  print('Recon Message Received for Pilot name: ' .. tbl.pilot)
  
  if reconDB[tbl.pilot] == nil then
    reconDB[tbl.pilot] = {}
    reconDB[tbl.pilot].messages = {}
  end
    
  table.insert(reconDB[tbl.pilot].messages, tbl)
end
...
--				cqch:send {
    ...
    elseif msg_id == 5 then
      -- Recon message
      --print('Recon Message Received.')
      
      AppendReconMessage(lua_table)
...
local function formMessageString(pilotName,runit)
  local outputString = runit.type .. ', location: ' .. runit.ll .. ', mgrs: ' .. runit.mgrs .. ', elev: ' .. round2(runit.elev*3.28084,0) .. ' feet. (' .. pilotName .. ')'
  return outputString
end

local function SendMessage(msg)
  
  coroutine.wrap(function(msg) ch:send(msg)  end)(msg) -- Run the coroutine
  
      
end

local function SendReconReport(report)
  ...
  msgString = 'Reconaissance Report: ```'
  for i, j in pairs(report) do
    msgString = msgString .. j .. '\n'
  end
  msgString = msgString .. '```'
...
local igu = {'Paratrooper','MAMPilot','soldier', 'Soldier', 'Infantry'}
  
  
local function ignoreUnit(runit)  
  
  for i,j in pairs(igu) do
  
    if string.find(runit.type, j) then
      return 1
    end
...
local function ReconReport()  
  local report = {}
  local counter = 0
  for pilotName, v in pairs(reconDB) do
        
    for i, j in pairs(reconDB[pilotName].messages) do      
      for m, runit in pairs(j.units) do
                
        if reconDB[pilotName].messages[i].units[m].r == 0 then
          
          reconDB[pilotName].messages[i].units[m].r = 1
                       
          if ignoreUnit(runit) == 1 then
          else 
            counter = counter + 1                  
            local msgString = formMessageString(pilotName,runit)
            table.insert(report, msgString)          
          end
          
          if counter > 9 then
            SendReconReport(report)
            return
          end
          
        end -- reconDB[pilotName].messages[m].r == 0
      end -- for m, runit in pairs(j.units) do
    end -- for i, j in pairs(reconDB[pilotName].messages) do
  end -- for pilotName, v in pairs(reconDB) do
  
  SendReconReport(report)
end

timer.setInterval(5000, ReconReport)